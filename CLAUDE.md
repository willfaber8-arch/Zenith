...
```ts
const count = useLiveAssignmentBadges()   // also calls setBadge('calendar', n)
const { done, total } = useHabitRingBadge()  // also calls setRingBadge('habits', done, total)
```

Both are side-effecting hooks mounted once in `BadgeSyncEffect`; they return
their number for anything that wants to display it inline as well.
...
56. **CosmeticPointsIndicator is the global Games DB seeder** (on a phone, `MobileTopbar` is — see rule 107) — `CosmeticPointsIndicator.tsx` calls `seedGamesDatabase()` on mount (idempotent). Because it lives in the Topbar (always mounted when authenticated), the Games DB is seeded before any game component needs it. Do NOT add `seedGamesDatabase()` calls to other global components — one seeder is sufficient. Game-specific components may call it locally only if they render without the Topbar (e.g., in tests).
...
88. **`seriesUid` is how "this event" and "all events" differ** — never add a second mechanism for recurrence scope. `lib/calendarMutations.ts` is the only place that decides which rows an edit or delete reaches, and it also picks the table: personal events live in `personalEvents`, imported ones in `calendarEvents`. A series edit carries the *time of day and duration* across occurrences but never the date — see rule 100, which corrects the earlier blanket ban on carrying times at all.
...
93. **Habit amounts can be fractional — round on write, format on read** — `stepAmount`, `targetCompletions` and `HabitCompletion.count` are no longer integers: half a mile and 2.5 litres are reasonable things to tick off. `utils/habitAmount.ts` owns this. `addHabitProgress` stores `roundAmount(prev + amount)` (3 dp) because ten steps of 0.1 sum to 0.9999999999999999 in binary floating point — unrounded, the goal is never met, `completedNow` never fires and the streak never increments while the row displays a full-looking count. Never interpolate a count straight into JSX; use `formatAmount`, or a partial reads "0.30000000000000004 / 1". The form holds the step and goal as **raw text** and parses with `parseAmount` — the old `type="number"` + `Math.max(1, Number(value))` coerced "" to 1 on every keystroke, so the field could not be backspaced and a decimal could not be typed at all (you cannot reach "0.5" without passing through "0"). `parseAmount` returns null for a half-typed field so the caller shows the text rather than substituting a value; an empty step falls back to 1 on blur. Integers round-trip unchanged, so every habit created before this behaves exactly as it did.

94. **Skipping a habit for a day makes it not-due, not a logged non-event** — `Habit.skippedDates?: string[]` (non-indexed field, no migration, same pattern as `color`). `isHabitScheduledOn()` in `utils/habitSchedule.ts` checks it before anything else, so every existing reader that already asks "was this due?" — the streak-continuity walk, the weekly consistency ratio in `computeCompletionSeries`, `addHabitProgress`'s own gate, `limitStreak`'s window — treats a skip exactly like an ordinary off day with zero new logic of its own. Toggle it with `toggleHabitSkip()` in `lib/habitSync.ts` (exposed as `toggleSkip` from `useHabits()`); it writes only `skippedDates`, nothing streak-related, because nothing else needs writing. It refuses to skip a day that already has a completion logged (`count > 0`) — a skip is a decision made instead of doing the habit, never a way to make logged progress stop counting — and un-skip has no such guard. `previousScheduledDate()` walks straight past a skipped date for every frequency: for `daily`/`specific_days` this falls out for free (its inner loop already calls the now-skip-aware `isHabitScheduledOn`), but `biweekly`/`monthly` land on a single arithmetic date and need their own bounded walk (`MAX_SKIP_WALK`) past a skipped occurrence. `detectBrokenStreaks()` was rewritten around this same primitive — `lastCompletedDate === previousScheduledDate(habit, today)` — instead of a raw calendar-day gap; besides making skips transparent to it, this incidentally fixed a real pre-existing bug where a `specific_days` habit got flagged broken on its own scheduled day before that day's occurrence had even happened. **UI note:** a skipped-today habit must stay visible in `HabitsView`'s daily list with its own row state (`DayStatus.skipped`, kept separate from `scheduled`) rather than disappearing the way a genuine off-day habit does — `visibleHabits` explicitly OR's in `isSkippedOn` alongside `isHabitScheduledOn` for exactly this reason. Colour it soft violet (`#a78bfa` at low alpha), never the row's red/green vocabulary — a skip is a choice, not a miss or a win.

95. **`HABIT_SOURCES` in `lib/habitSync.ts` is the whole auto-link registry — six entries now: `cardio`, `study`, `vocab`, `mood`, `reading`, `plant`.** A tracker joins it in exactly two steps: add the id to the `HabitAutoSource` union and its `{id, label, icon, unit, hint}` to `HABIT_SOURCES`, then call `void syncHabitSource(<id>, amount)` at the one place the underlying event is logged (`WorkoutsView` for cardio, `usePomodoroStateMachine` for study, `useMentalHealthLog` for mood, `BookTrackerDashboard` for reading, `BotanistView`'s `logWatering()` for plant). `syncHabitSource` does the rest — finds every habit with a matching `autoSource` and routes each through `addHabitProgress`, the same engine the manual "+" button uses, so a linked habit's completion/streak rules never drift from a tapped one's. Never call `addHabitProgress` directly from a tracker; always go through `syncHabitSource` so the habit-day default and the `zenith:habit-complete` toast event stay centralised. **Watering a plant** (`BotanistView.tsx`'s `PlantCard`) is the reference example of pairing an auto-source with one-shot confirmation animation: a `justWatered` boolean (cleared on a `setTimeout`, itself cleared on unmount via `useEffect`) drives three CSS-only, non-looping cues together — a `plantWaterPulse` ring on the card, the water button swapping to "Watered!" with a check icon, and a small `dropletBurst` of falling droplet glyphs positioned via a `.waterBtnWrap { position: relative }` wrapper. The `animation-fill-mode: both` on the droplets is safe specifically because they are leaf elements (nothing nested inside that a `position:fixed` descendant could get trapped in — see rule about `.anim-scale-in`/`.anim-slide-in`) and because the whole burst unmounts with `justWatered` rather than persisting. A toast (`useToast`) backs up the visual cue for anyone who missed it.
...
96. **Calendar events can meet on specific weekdays with per-day hours, carry a priority, and edits can be scoped like deletes.** Three additions to `NewEventModal` in `components/views/CalendarView.tsx`, sharing the surrounding machinery rather than inventing parallel ones:
    - **Weekday picker** — the Repeats `<select>` gains a `Choose specific days…` option (`CUSTOM_DAYS_REPEAT` from `lib/personalEventSeries.ts`) that reveals a day-chip + uniform/per-day time UI, deliberately mirroring `UniversityScheduleReplicator`'s `days`/`dayTimes`/`uniformTimes` pattern rather than a new one. Unlike that component (Mon–Fri only), the picker here covers all seven days (`DOW_LIST`, `dow` 0–6 matching `Date#getDay()`, matching `CustomDayMeeting.dayOfWeek`). Occurrences are planned by `planCustomDayOccurrences()` (walks one calendar day at a time from an anchor date, exactly like `scheduleGenerator.ts`'s `planSessions()` — never through `expandOccurrences`, which cannot express two different times in one rule) and written by `createCustomDayEvent()` / `repeatExistingEventCustomDays()`. When this mode is active the modal hides the ordinary Date/All-day/Start/End fields (they'd otherwise silently do nothing, since the per-day times override them) and relabels the date field "Starting from *" — it becomes the planner's anchor date, not an event time.
    - **Priority + overlap cascade** — `EventPriority = 'low'|'normal'|'high'` (`lib/db.ts`, on both `CalendarEvent` and `PersonalEvent`; absent counts as `'normal'`). `utils/eventOverlap.ts`'s `layoutOverlaps()` groups events into overlap clusters (interval-merge sweep, zero-duration events treated as a 20-minute slot) and ranks each cluster by priority desc, then start time asc — never merges pills into one. `WeekGrid` looks up each event's `{stackIndex, clusterSize}` and `EventPillEl` turns that into a left/right inset cascade (`OVERLAP_CASCADE_PX` per step) plus `z-index`; hover/focus sets `zIndex: 1000` to bring the covered one fully forward. Transform is deliberately never used for this cascade — only `left`/`right`/`box-shadow`/`z-index` — since a `transform` on an ancestor becomes the containing block for any `position:fixed` descendant, and event pills are exactly the kind of element other overlays can end up nested near.
    - **Edit scope** — `EditScope` (`lib/calendarMutations.ts`) widened from `'this'|'series'` to `'this'|'future'|'series'`; `targetIds()` filters `'future'` to `r.startMs >= event.startMs` on the already-fetched series rows. `NewEventModal` shows a scope picker ("This event" / "This & following" / "All events") whenever `inSeries`, defaulting to `'this'`; `EventDetailPopover`'s delete confirmation grew the same third button. `lib/calendarUndo.ts`'s `rowsInScope()` mirrors `targetIds()` exactly so an undo snapshot always covers the same rows the mutation touched — fixed alongside this, since it's the same function: personal events reached it with the grid's *negated* id (`pe.id * -1` — see `personalRowId()` in `calendarMutations.ts`) and `rowsInScope` was not taking `Math.abs()` of it, so `personalEvents.get(-3)` matched nothing and captured an empty snapshot — every personal-event edit or drag silently produced no "Undo" at all. Never bypass `targetIds()`/`rowsInScope()` with a direct `seriesUid` query; the two have to keep agreeing on what a scope means.

97. **A habit tap can be undone, but only by holding, and only the last one.** `components/ui/HoldToConfirm.tsx` is a small reusable button that fires `onConfirm` only after being held ~700ms (CSS `transition: transform var(--hold-ms) linear` on a `scaleX(0)→scaleX(1)` fill, timed to match a `setTimeout` of the same duration — no rAF polling). It exists because `ConfirmDelete`'s "press again to confirm" is the wrong shape here: the gesture that would confirm an accidental tap (another tap) is exactly the gesture that caused it, so it protects against nothing. Use `HoldToConfirm` instead of `ConfirmDelete` specifically when the action being guarded is easy to re-trigger by the same input that confirms it; keep using `ConfirmDelete` everywhere else. The undo itself is snapshot-based, not arithmetic: `captureHabitTap()`/`revertHabitTap()` (`lib/habitSync.ts`) capture and restore the whole `habits` row plus that day's `habitCompletions` row via `put` (matching `lib/calendarUndo.ts`'s approach) — a tap's effect on `streakCount`/`lastCompletedDate` can't be reversed by re-deriving it after the fact, because whether it continued a streak or started a fresh one after a break is a decision already thrown away by the time "undo" is pressed. `useHabits().increment()` captures the snapshot *before* calling `addHabitProgress` and returns it as `result.undo`; `HabitsView` keeps at most one snapshot per habit in ephemeral (unpersisted) React state — a second tap replaces the entry, using it clears it, and a day rollover (`today` changing) clears all of them, so this is for catching a mis-tap a moment after it happens, not for rewriting history. The control occupies the exact same `habitRight` slot as the Skip button and never appears alongside it: Skip only shows before anything is logged (`habit.todayCount === 0`), Undo only after (`habit.todayCount > 0` and a snapshot exists) — same real estate, conditions that never overlap.

98. **A sidebar item says either "how many are waiting" or "how much of today is done" — never both, and never a third thing.** `NavBadgeContext`'s `BadgeMap` holds a discriminated `NavBadge`: `{kind:'count'}` for a list that drains (the Calendar's tasks, via `setBadge`) and `{kind:'ring'}` for a fixed set that fills (today's habits, via `setRingBadge`). `NavLinkItem` renders whichever it is given and knows nothing about either domain — push new sidebar state from a hook in `BadgeSyncEffect`, the way `useLiveAssignmentBadges` and `useHabitRingBadge` do, rather than reaching into the sidebar from a view. The ring's SVG strokes `currentColor` so it inherits the nav item's `--item-accent` per category for free, and only changes colour in one state: everything due today done, in the same `--accent-green` a finished habit row uses. **The number it shows is not its own.** `useHabitRingBadge` reads `doneCount`/`scheduledCount` straight off `useHabits()` instead of counting habits itself, because the denominator has real rules in it — habits not scheduled today are excluded, skipped ones stop counting (rule 94), and a limit habit is never "done" while the day runs — and all of them already live in that hook, which is what the Habits view displays. A sidebar reading 3/8 beside a page reading 3/5 is worse than no sidebar number at all; this is rule 91's "every reader agrees with the writer" applied to a second surface. `setRingBadge` bails out when `done`/`total` are unchanged, since it runs from an effect on live-query results that re-resolve on every write to the table and would otherwise re-render the whole sidebar for an identical ring. `useHabitProgress` was deleted here rather than kept: it was unused and counted `lastCompletedDate === today` over *all* habits, so it would have disagreed with the page on exactly the days the rules matter.

99. **A repeat that was never grouped is repaired by filling in `seriesUid`, not by adding a second way to group.** Reported as "the save for future events doesn't work — if I click on one it won't change the rest". The cause was that `utils/scheduleGenerator.ts` wrote one independent `calendarEvents` row per class session and never stamped a `seriesUid`, so a semester of one course was forty unrelated events: `seriesSize()` answered 1, the scope picker never appeared, and every scope collapsed to the single clicked row in `targetIds()`. Two halves to the fix, and the constraint on both is rule 88 — `seriesUid` stays the only mechanism. **Going forward**, the generator stamps one id across the whole run, scoped to the run (`${uidPrefix}-${now}`) rather than to the course name, so retaking a course next term is its own series and not an edit surface reaching back into last term. **Backwards**, `lib/calendarSeriesBackfill.ts` reconstructs the grouping once per install (`zenith_series_backfill_v1`, fired from a ref-guarded effect in CalendarView so it happens on the screen that shows the events, and announces itself in a toast). Its grouping is deliberately asymmetric about risk: missing a group leaves an event editable one at a time, which is where it already was, but inventing one puts "All 40" in front of a delete button. So imported rows group on **feed + title** (a feed is a machine-made set; the feed is in the key so "Lecture" in two courses stays apart), personal rows group only on **title + time-of-day + duration + allDay** (nothing made those a set — they were typed in one at a time, so identical-looking is the only honest evidence), and a key matching one row is never a series. `planSeriesBackfill` is pure and takes the grouping rules; `runSeriesBackfill` only ever fills a gap — rows that already have a `seriesUid` are untouched, which is what makes re-running it a no-op.

100. **A repeat's time of day belongs to the series; its date belongs to the occurrence.** Reported as "I made a freetime event to repeat everyday, but I set it at the wrong time — I'd have to adjust every single future event." `applyEventPatch` used to strip `startMs`/`endMs` from anything but the clicked row, and the reasoning only went halfway: an occurrence's `startMs` is an absolute instant, so writing one onto forty rows really does collapse a term onto one afternoon — but the *clock reading* is not per-occurrence at all, and it is the thing people actually need to fix. `utils/seriesEdit.ts` splits the two. `planSeriesTimes(mode, …)` is pure and takes `'occurrence-only'` (dragging, always — a drag is a gesture on one block of time, never a statement about the series), `'time-of-day'` (every row takes the new clock reading and duration on its own date) or `'shift-days'` (as above, plus a whole-day slide). Every date step goes through local calendar components, never ±86,400,000 ms, because "same time next week" has to survive a DST boundary. **The form and the write run the same planner over the same rows** — `seriesOccurrences()` feeds the preview, `applyEventPatch` feeds the write — so the sentence under the scope picker cannot promise something the save then does differently; that is rule 91's "every reader agrees with the writer" applied to a preview. Two asymmetries are deliberate: under `'series'` an occurrence that already happened keeps its time (`protectBeforeMs`, start of local day) because rewriting when last week happened turns the calendar's record into a guess — but the *edited* occurrence always moves, since it is the one being looked at, and `'future'` needs no such guard because the user already picked where to start. And non-time fields still reach the past: a rename is a property of the thing, not a record of when it happened.

101. **Ask only where the ambiguity actually is.** The series-edit prompts follow one rule: a change with a single sensible reading never interrupts. Changing the time is unambiguous, so it just says what it will do. Changing the *date* as well is not — "this Tuesday moved" and "the whole pattern moved" are both reasonable — so that, and only that, raises a second choice (`dateMode`, defaulting to the safer "only this one's date"). Saving something that reaches more than `BIG_SERIES_EDIT` (10) occurrences arms an in-place confirm in the actions row rather than a dialog over the dialog, matching how `ConfirmDelete` asks, so the form stays readable behind the question. Counts shown to the user come from the planner's output, never from a second count of the rows — the preview says how many *move*, while the toast afterwards says how many rows were *written*, and those two numbers genuinely differ when the past is held back or a rename reaches further than a time change does.

102. **An event pill must be opaque.** `.eventPill` was `${color}2e` — 18% alpha — so two events at the same time showed through each other and through the grid, and reading either one was guesswork. Offsetting them sideways (rule 96's cascade) only helps if the pill in front is solid, which is why "the priority doesn't really work, the events blend together" was still true after the cascade shipped. The tint is now mixed opaque in CSS: `color-mix(in srgb, var(--pill-color) 32%, var(--surface-card))`, with a plain `background: var(--surface-card)` declaration before it as the fallback — losing the event's tint is survivable, going transparent is not. Mixing in CSS rather than blending to a fixed hex in JS is what lets a cosmetic theme that redefines `--surface-card` retint every event for free (rule 71). The colour itself arrives as the `--pill-color` custom property on the element. Month-view chips are deliberately left translucent: they are a list, never stacked, so nothing can blend there.

103. **An IP address is compared as bytes, never as text.** `isBlockedIpv6` matched the *spelling* of an address — a regex for a dotted quad inside `::ffff:`, a `startsWith('fc')` — and `new URL()` has already normalised the spelling away by the time the guard sees it. `http://[::ffff:127.0.0.1]/` arrives as `::ffff:7f00:1`, which no dotted-quad regex can match, so `cal-proxy`, `book-cover` and `recipe-import` would each fetch `169.254.169.254` (cloud instance metadata), `192.168.x.x` and loopback on request. `ipv6ToBytes()` in `lib/server/ssrfGuard.ts` now parses to a 16-byte array — `::` expansion, an embedded IPv4 tail, a `%zone` suffix — and every rule is a byte comparison: an IPv4-mapped address is unpacked and handed to `isBlockedIpv4`, so the two families cannot disagree about what `127.0.0.1` means, and `::/96` is blocked whole rather than enumerating `::1` and `::`. The test file is the specification: it lists every spelling of loopback and metadata that must be refused *and* asserts real public addresses still resolve, because a guard that blocks everything passes a one-sided test while quietly breaking the feature. Never add a rule that looks at the string.

104. **A payload from a peer is a stranger's input, and the peer id is the only thing you know about them.** `useFriendsNetwork`'s `conn.on('data')` trusted `payload.senderId` for the key it wrote under, so any connected peer could name themselves as someone else and overwrite that friend's leaderboard row — and a payload missing `snapshot` threw inside `evaluateTemporalSnapshot` as an unhandled rejection that took the handler with it. `utils/peerPayload.ts` holds the whole rule as a pure function: `sanitiseSyncPayload(raw, conn.peer)` refuses anything whose claimed `senderId` is not the id the connection actually arrived on, clamps names/titles/uids to fixed lengths, strips control characters, coerces every count to a finite non-negative number, and caps the shared event list at `MAX_SHARED_EVENTS` so a flood cannot fill IndexedDB. It returns `null` rather than throwing and the handler drops a `null` silently — a malformed message from a friend is not an error state for the user. Validate in the pure function, never inline in the handler, so the rules can be tested without a network.

105. **A once-a-second tick belongs in the leaf that shows it.** The Topbar clock kept `now` in Topbar's own state, so `setNow` re-rendered the whole status cluster 86,400 times a day to change two digits — including `NotificationBell`, whose hook read and `JSON.parse`d localStorage three times in its render body on every one of those renders. Both halves generalise: state that changes on a timer lives in the smallest component that displays it (`TopbarClock`), and anything a hook reads out of localStorage is memoised on the store's own version counter rather than re-read per render (`useNotificationCenter`'s `version`, bumped by `subscribeNotifications`). Derived values follow the same rule — `unseen` filters the list already in hand instead of calling `unseenCount()`, which re-read, re-pruned and re-sorted the store a second time to answer.

106. **A file nothing imports is deleted, not kept "just in case".** Twenty-nine modules and their CSS — ~5,700 lines, including six whole views, a Leaflet map and a cosmetics shop — were reachable from nothing but comments in other files mentioning them by name, which is what made them look alive to grep. They cost nothing at runtime (an unimported module is in no chunk) but they cost every search, every refactor and every audit, and one of them was the only reason `leaflet`, `react-leaflet` and `@types/leaflet` were installed on every CI run. Verify orphanhood by import path, then confirm the remaining hits are prose rather than code, then delete the module *and* its `.module.css`, the dependencies only it reached, and any build config written for it (the `vendor-leaflet` splitChunks group). Typecheck, the unit suite and a production build are the safety net — if all three pass, nothing was using it.

107. **A phone is a different composition of Zenith, not a narrower desktop.** Below 767px Zenith is five screens — Home, Today, Habits, Tasks, Notes — plus Settings behind the avatar, and nothing else is reachable. `lib/phoneViews.ts` is the whole decision: `PHONE_TABS` is the bottom bar, and `phoneViewFor(view)` maps every other view to Home. It **never rewrites navigation**: ViewRouter and the bar both run the stored view through it, so a laptop still reopens the Arcade where you left it while the phone shows Home. AppShell reads `usePhoneLayout()` (synchronous `matchMedia`, safe only below the auth check because AppShell never server-renders — see its note) and on a phone renders no sidebar at all, `MobileTopbar` instead of `Topbar` (page name + avatar menu with Settings and Sign out), and `MobileTabBar`. The frame is fixed (`.shell.phoneShell`): top bar and bottom bar never move, the viewport is a flex column, and a phone screen takes the space between the bars with `flex: 1; min-height: 0` and scrolls *only its lists* inside themselves — the page itself never scrolls, vertically or sideways. `tests/phoneLayout.spec.ts` asserts exactly that (document size equals viewport, nothing past the edge) on every screen. Three consequences worth knowing: `MobileTopbar` seeds the Games DB because the desktop seeder never mounts on a phone (rule 56); the bar slides away while a text field has focus so it cannot ride up over the keyboard; toasts sit above the bar on a phone, never over it. Desktop code paths are untouched by any of this — the branch that shipped it pixel-diffed identical against master on Home, Habits, Tasks, Notes and Outlook at 1440×900. Keep it that way: phone behaviour goes behind `usePhoneLayout()` or a class it sets, never into a shared rule.

108. **A phone screen reuses the desktop's writes and the desktop's numbers.** Home, Today and Tasks are phone compositions (`components/phone/`), but none of them has rules of its own. Today ticks a habit with `useHabits().increment` (so streaks, the cutoff and the undo snapshot behave exactly as on the Habits page, including rule 97's hold-to-undo) and a task with `setDone` (so a repeating task moves to its next date); its task and event lists come from `planAgenda` in `lib/hooks/useTodayAgenda.ts`, which mirrors CalendarView's visibility rules — hidden calendars and switched-off feeds stay hidden. Home's tile counts are not counted on Home: Habits and Tasks read the sidebar's own badges from `NavBadgeContext` (which BadgeSyncEffect keeps live with or without a sidebar on screen), and Today reads `useTodayAgenda`, so the tile cannot disagree with the screen it opens (rule 98). A badge is *removed* at zero, so an absent badge means none, not loading. Tasks on a phone is `TasksPanel phone` — the same component and writes as the Calendar's Tasks tab, with the roadmap hidden and the composer rearranged by CSS `order` alone; there is still one task list (rule 92). Habits and Notes are the ordinary views, phone-aware via `usePhoneLayout()`: Habits pins a one-line header and tucks analytics behind a closed Stats toggle; Notes shows list *or* editor (`data-open`), and its Back flushes any pending autosave before leaving so nothing typed is lost.

109. **A "do this once" flag is claimed before the first await.** `ensureGeneralHabitPreset` read its seeded-flag, awaited a count of `habits`, and only then set the flag — so two calls in flight together (React's dev double-effect, or two tabs on first run) both passed the check, both counted zero, and every starter habit arrived twice. It now checks-and-sets the flag in one synchronous step, and does count-then-`bulkAdd` inside a single `rw` transaction, which IndexedDB serialises even across tabs where localStorage cannot. Any one-time seed follows the same two rules: claim synchronously, and make the check and the write one transaction. It does not clean up duplicates an earlier run already made — deleting habit rows on a guess is not this function's call. `seedUserProfile` had the same shape (get, then add) and now reads and writes inside one `rw` transaction too.

110. **"Already showing it" still has to be visible.** ViewRouter fades the old view out for 200ms before swapping in the new one. Navigating away and straight back inside that window started the fade (`visible=false`), cancelled the swap, and then took the early return for "already displayed" without ever fading back in — the page sat at opacity 0 with pointer events off until you navigated again. Two quick taps on the phone's bottom bar reproduced it; so did a fast double-click in the desktop sidebar. The early return now sets `visible` back to true. Any transition that can be cancelled mid-flight needs its "nothing to do" branch to restore the resting state, not merely skip the work.

111. **Cloud sync runs by itself, and every automatic step is one that cannot lose data.** `lib/CloudSyncContext.tsx` is the single controller, mounted once by AppShell so it runs on every screen of phone and desktop — it used to live inside the Settings panel's hook, so nothing synced anywhere else. It saves ~3 s after a change and immediately on `visibilitychange→hidden`/`pagehide`; it checks the cloud on open, on return to the app, on `online` and every 60 s while visible. What it *does* at each check comes from `decideSync()` in `services/cloudSnapshot.ts`, which is pure and takes the whole situation: it only ever chooses **push** (this device changed, the cloud still holds what this device last saw) or **pull** (the cloud changed, this device has nothing unsaved) on its own, and everything else is `blocked` with a reason the banner (`components/CloudSyncBanner.tsx`) explains and asks about. A pull reloads the page, because a screen holding a draft would otherwise save its stale copy straight back — so the 60-second poll only *offers* a pull (`updateReady`) rather than reloading under someone reading, and the first check after a load never auto-loads again (`justLoaded`), so a watermark that failed to save cannot become a reload loop. The guards, each tested in `__tests__/sync/`:
    - **Compare-and-swap push.** The UPDATE is filtered on the server-stamped `updated_at` this device last saw; zero rows matched means someone else wrote first → `conflict`, nothing overwritten. It used to be an unconditional upsert, which let a stale device replace everything the moment anything changed on it.
    - **Insert-only first push.** A device with no watermark INSERTs; if a copy appears in the same instant the primary key rejects it → `conflict`. A never-synced device never replaces an existing copy.
    - **Account lock.** `SnapshotMeta.ownerUserId` records whose data this is; a different signed-in account blocks both directions (`account-mismatch`). The banner offers only "load this account's data" (with a safety copy) — never uploading one account's workspace into another's.
    - **Version guard.** An app older than the cloud copy's `schema_version` may not save over it (`outdated-app`); a cloud load passes `preserveMissingTables` so a copy from an older app cannot empty a table it never heard of.
    - **Mass-deletion guard.** `looksLikeMassDeletion(lastSyncedRowCount, now)` — losing ≥50% and ≥25 rows — holds the push (`mass-deletion`) so cleared browser storage does not propagate to every device; the banner offers to bring it back or confirm.
    - **One tab at a time** via `navigator.locks` (`withSyncLock`); a tab that loses the race reports `busy` and stays quiet.
    Never add an automatic path around `decideSync`, and never make a guard resolve itself: a safeguard that decides for the user is a data-loss path with extra steps.

112. **Nothing leaves this device that belongs to this device.** `utils/deviceSecrets.ts` is the single list both `buildBackupPayload` and `importJsonToLocalDatabase` consult, for the cloud copy and a downloaded file alike: the AI provider key (`zenith_ai_config_v1`), the Google Books key, the cached location, the sync client id, and the letterbox key pair on `userProfile` (`letterboxPublicKeyJwk`/`letterboxPrivateKeyJwk`) — plus anything whose *name* says it is a credential (`isDeviceOnlySetting`'s pattern), so a key added next month is covered the day it is written. All of these were being uploaded in every snapshot, against their own modules' "never leaves the device" comments. Both directions: stripped on the way out, ignored on the way in, and this device's own values kept across a load — inside the import transaction for the profile fields — because older copies still contain someone else's. Copies made since carry `secretsStripped: true`; `getRemoteMeta` reads that one key (`secrets_stripped:payload->>secretsStripped`) and `decideSync` pushes a clean copy over any cloud copy without it, even when nothing changed, so a leaky copy from before this does not sit in the cloud forever. The app's CSP already refuses connections to anything but real Supabase hosts; keep it that way.

113. **Nothing is replaced without a copy of it kept first.** A cloud load stores a `before-cloud-load` safety copy of this device, and "keep this device's version" stores the cloud's copy as `cloud-copy`, before either is overwritten (`storeSafetyCopy` in `utils/dbSnapshots.ts`). If the copy cannot be stored — quota, too large — the load does not happen (`no-backup`) unless the user explicitly chooses to go ahead. `DbSnapshot.kind` (non-indexed, no migration) keeps `KEEP_SNAPSHOTS` of *each* kind, so a day of switching between devices cannot push the daily copies out; the daily schedule counts only `daily` and `manual` copies, since a sync safety copy holds what was about to be replaced, not what you have now. Settings → Local snapshots labels each by kind.

114. **Sync state never compares one machine's clock with another's.** "Unsaved" is simply `lastLocalChangeAt != null` — every successful push and pull clears it, and a push that raced a new edit leaves it set. It used to be `lastLocalChangeAt > lastSyncedAt`, device clock against server clock, so a phone running a minute slow treated an edit made just after a sync as already saved, never pushed it, and the next load replaced it. "Has the cloud changed?" is likewise identity, not ordering: `sameVersion(meta.lastSyncedAt, remote.updatedAt)` against the server-stamped value. Any new sync logic follows the same rule — flags and server-issued versions, never cross-machine timestamps.
